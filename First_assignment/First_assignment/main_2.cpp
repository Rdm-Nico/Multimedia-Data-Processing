...
struct vector {
    ...
    vector(const vector& other) {
        ...
        for (size_t i = 0; i < n_; i++)
        {
            ...
            // la memcpy andava pi� forte perch�: usare accelerazione hardware pi� forti
        }
    }
    ...
    /*
        Una versione in cui non si pu� modificare il valore ritornato ( la seconda )
        Una versione in cui si pu� utilizzare il valore ritornato( la prima)
    */
...
int main(int argc, char* argv[])
{
    ...
    // serve una nuova modalit� di allocare memoria 
    Si utilizzer�:*/
    numero* pn = new  numero;
    int q = pn->val_;
    // non c'� la garbage collection --> si deve liberare la memoria 
    delete pn;

    // la sintassi per allocare pi� di un oggetto insieme:
    ...
    */    
    /*NON SI PUO FARE v[0] = 5  , perch� deve essere una variabile --> essere un indirizzo di memoria, si inserisce una reference per farlo diventare un l-value*/
