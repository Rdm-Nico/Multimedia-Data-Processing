...
#include<vector>
//  4 version of the @main.cpp item with information hiding and 

// tutte le cose nella libreria std sono dentro al propio namespace 
...
// impostare il proprio namespace
namespace mpd {
    template<typename T>
    class vector { // class = struct . L'unica differenza è la visibilità di defalut 
    private:
        T* nums_;
        int n_;
    ...
    public:
        ...
        friend void swap(vector& left, vector& right) {
            using std::swap; // se implemento una swap è sono dentro un tamplate(che quindi lavora con tipi generici) --> using ( porta quella funzione e la rende disponibile)
